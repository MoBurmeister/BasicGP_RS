...
multi_multitaskgp = MultiMultitaskInitializer(dataset_manager=main_dataset)

multi_multitaskgp.initially_setup_model()

multi_multitaskgp.train_initially_gp_model()

#reference point handed over as negative values!

reference_point = torch.tensor([-1500.00, 100.00], dtype=torch.float64)

bayesian_optimizer = BayesianOptimizer(multiobjective_model=multi_multitaskgp, 
                                       bool_optional_ending_optimization_each_iteration=True,
                                       reference_point=reference_point,
                                       save_file_name="TL_opt_Multitask")
